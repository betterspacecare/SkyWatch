/**
 * Script to generate constellation data from Stellarium stick figures
 * Uses Hipparcos star catalog for coordinates
 * 
 * Run with: npx ts-node scripts/generate-constellations.ts
 */

import * as fs from 'fs';
import * as path from 'path';

// Constellation full names mapping
const constellationNames: Record<string, string> = {
  "Aql": "Aquila", "And": "Andromeda", "Scl": "Sculptor", "Ara": "Ara",
  "Lib": "Libra", "Cet": "Cetus", "Ari": "Aries", "Sct": "Scutum",
  "Pyx": "Pyxis", "Boo": "Boötes", "Cae": "Caelum", "Cha": "Chamaeleon",
  "Cnc": "Cancer", "Cap": "Capricornus", "Car": "Carina", "Cas": "Cassiopeia",
  "Cen": "Centaurus", "Cep": "Cepheus", "Com": "Coma Berenices", "Cvn": "Canes Venatici",
  "Aur": "Auriga", "Col": "Columba", "Cir": "Circinus", "Crt": "Crater",
  "CrA": "Corona Australis", "CrB": "Corona Borealis", "Crv": "Corvus", "Cru": "Crux",
  "Cyg": "Cygnus", "Del": "Delphinus", "Dor": "Dorado", "Dra": "Draco",
  "Nor": "Norma", "Eri": "Eridanus", "Sge": "Sagitta", "For": "Fornax",
  "Gem": "Gemini", "Cam": "Camelopardalis", "CMa": "Canis Major", "UMa": "Ursa Major",
  "Gru": "Grus", "Her": "Hercules", "Hor": "Horologium", "Hya": "Hydra",
  "Hyi": "Hydrus", "Ind": "Indus", "Lac": "Lacerta", "Mon": "Monoceros",
  "Lep": "Lepus", "Leo": "Leo", "Lup": "Lupus", "Lyn": "Lynx",
  "Lyr": "Lyra", "Ant": "Antlia", "Mic": "Microscopium", "Mus": "Musca",
  "Oct": "Octans", "Aps": "Apus", "Oph": "Ophiuchus", "Ori": "Orion",
  "Pav": "Pavo", "Peg": "Pegasus", "Pic": "Pictor", "Per": "Perseus",
  "Equ": "Equuleus", "CMi": "Canis Minor", "LMi": "Leo Minor", "Vul": "Vulpecula",
  "UMi": "Ursa Minor", "Phe": "Phoenix", "Psc": "Pisces", "PsA": "Piscis Austrinus",
  "Vol": "Volans", "Pup": "Puppis", "Ret": "Reticulum", "Sgr": "Sagittarius",
  "Sco": "Scorpius", "Ser": "Serpens", "Sex": "Sextans", "Men": "Mensa",
  "Tau": "Taurus", "Tel": "Telescopium", "Tuc": "Tucana", "Tri": "Triangulum",
  "Tra": "Triangulum Australe", "Aqr": "Aquarius", "Vir": "Virgo", "Vel": "Vela"
};

// Stellarium constellation stick figures (from celestialprogramming.com)
// Format: [Abbr, numLines, HIP1, HIP2, HIP3, HIP4, ...] - stars taken in pairs
const stellariumLines: (string | number)[][] = [
  ...
  ["Ori",23 ,26727,26311,26311,25930,29434,29426,29434,28716,28716,27913,29426,29038,29038,27913,29426,28614,28614,27989,27989,26727,26727,27366,27366,24436,24436,25930,25930,25336,25336,26207,26207,27989,25336,22449,22449,22549,22549,22730,22730,23123,22449,22509,22509,22845,29038,28614],
  ...
  ["Vir",12 ,57380,60129,60129,61941,61941,65474,65474,69427,69427,69701,69701,71957,65474,66249,66249,68520,68520,72220,66249,63090,63090,63608,63090,61941],
...
import { StarCatalogAPI } from '../src/star-api';

// Star coordinates from Hipparcos catalog (HIP ID -> {ra, dec, mag, name})
// RA in degrees, Dec in degrees
const hipStars: Record<number, { ra: number; dec: number; mag: number; name?: string }> = {};

// Load stars from API
async function loadStarsFromAPI(): Promise<void> {
  console.log('Fetching star data from catalog API...');
  const api = new StarCatalogAPI();
  
  // Collect all required HIP IDs
  const requiredHipIds = collectRequiredHipIds();
  const hipIdArray = Array.from(requiredHipIds);
  
  console.log(`Fetching ${hipIdArray.length} stars...`);
  
  // Fetch in batches to avoid overwhelming the API
  const batchSize = 100;
  for (let i = 0; i < hipIdArray.length; i += batchSize) {
    const batch = hipIdArray.slice(i, i + batchSize);
    const stars = await api.getStarsByHipIds(batch);
    ...
    console.log(`Loaded ${Object.keys(hipStars).length} stars so far...`);
  }
}

// Collect all unique HIP IDs needed for constellations
function collectRequiredHipIds(): Set<number> {
  const hipIds = new Set<number>();
  
  for (const constellation of stellariumLines) {
    // Skip first two elements (abbr and numLines)
    for (let i = 2; i < constellation.length; i++) {
      hipIds.add(constellation[i] as number);
    }
  }
...
function generateConstellations(): any[] {
  const constellations: any[] = [];
  
  for (const data of stellariumLines) {
    const abbr = data[0] as string;
    const hipIds = data.slice(2) as number[];
    
    const lines: any[] = [];
    const allRAs: number[] = [];
    const allDecs: number[] = [];
    
    // Process pairs of stars
    for (let i = 0; i < hipIds.length; i += 2) {
      const hip1 = hipIds[i];
      const hip2 = hipIds[i + 1];
      
      if (hip1 === undefined || hip2 === undefined) continue;
      
      const star1 = hipStars[hip1];
      const star2 = hipStars[hip2];
      
      if (star1 && star2) {
        lines.push({
          star1: { hipId: hip1, ra: star1.ra, dec: star1.dec },
          star2: { hipId: hip2, ra: star2.ra, dec: star2.dec }
        });
        
        allRAs.push(star1.ra, star2.ra);
        allDecs.push(star1.dec, star2.dec);
      } else {
        // Log missing stars
        if (!star1) console.warn(`Missing star HIP ${hip1} for ${abbr}`);
        if (!star2) console.warn(`Missing star HIP ${hip2} for ${abbr}`);
      }
    }
    
    if (lines.length > 0) {
      // Calculate center
      const centerRA = allRAs.reduce((a, b) => a + b, 0) / allRAs.length;
      const centerDec = allDecs.reduce((a, b) => a + b, 0) / allDecs.length;
      
      constellations.push({
        id: abbr.toLowerCase(),
        name: constellationNames[abbr] || abbr,
        lines,
        centerRA: Math.round(centerRA * 100) / 100,
        centerDec: Math.round(centerDec * 100) / 100
      });
    }
  }
...
// Main execution
async function main() {
  console.log('Loading star data from API...');
  await loadStarsFromAPI();
  
  const requiredHipIds = collectRequiredHipIds();
  console.log(`Total unique HIP IDs needed: ${requiredHipIds.size}`);
  console.log(`Stars loaded: ${Object.keys(hipStars).length}`);
  
  // Find missing stars
  const missingHipIds: number[] = [];
  for (const hipId of requiredHipIds) {
    if (!hipStars[hipId]) {
      missingHipIds.push(hipId);
    }
  }
  
  if (missingHipIds.length > 0) {
    console.log(`\nWarning: ${missingHipIds.length} stars still missing`);
    console.log('Missing IDs:', missingHipIds.slice(0, 20).join(', '), '...');
  }
  ...
  // Write output
  const outputPath = path.join(__dirname, '..', 'data', 'constellations.json');
  fs.writeFileSync(outputPath, JSON.stringify(constellations, null, 2));
